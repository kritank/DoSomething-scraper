...
import { ArrowLeft, RefreshCw, Link2, Briefcase, User, Users } from 'lucide-react';
              ...
              {byType[key].length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {groupByCreator(byType[key]).map((group) => (
                    <CategoryAccountGroup key={group.key} group={group} />
                  ))}
                </div>
              )}
            </div>
...
function CategoryAccountGroup({ group }) {
  return (
    <div
