...
export default function PerformanceDecayCard({
  decay,
  loading,
  days,
  onDaysChange,
    ...
    <div className="card p-5 flex flex-col gap-4 min-w-0">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-1.5">
          ...
          <InfoTip text="Average views (or likes, when views aren't public) per hour, bucketed by how old each post was at the time it was scraped -- across every post published in this window. A falling curve is the expected shape: most of a post's lifetime engagement lands early." />
        </div>
      ...
      {loading ? <Skeleton className="h-60 w-full" /> : <PerformanceDecayChart decay={decay} color={color} />}
    </div>
  );
}
